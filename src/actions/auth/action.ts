...
export async function sendContactUs(data: contactUsType) {
	try {
		const response = await axiosInstance.post("contact/contact-us/", data);
		console.log("response for contact", response);
		return { ok: true, message: "Contact send successfully" };
	} catch (error: any) {
		return {
			ok: false,
			message: getErrorMessage(error),
